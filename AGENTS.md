...
- **Source pages need an inbound anchor too.** Like Decisions, a new `wiki/sources/` page is orphaned unless ≥1 *canonical* page links to it (`tofile/`, `index.md`, `overview.md` don't count). Always pair Source creation with ≥1 resolved-entity update — append a `## …mentions`/`## Clippings`/`## Docs` bullet `(from [[wiki/sources/<slug>]])` to an existing Person/Topic/Org/Project page. This is mandatory anti-orphan, not optional enrichment; `/kb-lint` flags the orphan otherwise.
- **Don't silently drop inbound-external email threads that match tracked entities.** The Outlook sender-domain filter drops inbound-only external mail (no own-domain sender). Before dropping, check whether the sender domain **or** normalized subject matches an existing `wiki/sources/` slug or canonical entity (e.g. a known vendor/sales thread). If so, fetch the body and decide by **content value** — a vendor counter-offer on a tracked decision is exactly what the vault captures. Apply value judgement consistently in both directions: recover high-value inbound mail the mechanical filter drops, and skip low-value invites the quality gate would keep (e.g. one-off talk announcements). Failure mode: the mechanical filter buried a vendor's Enterprise counter-offer tied to an existing non-renewal Decision.
